# authlog_api/api/v1/routers/routes/events.py
from typing import Optional, Literal, List
from fastapi import APIRouter, Depends, HTTPException, Query, Path, status
from sqlalchemy.orm import Session

from authlog_api.schemas.events import AuthLogCreate, AuthLogUpdate, AuthLogOut
from authlog_api.api.deps import get_current_user
from authlog_api.db.session import get_db
import authlog_api.api.v1.routers.crud.events as crud
...
def list_events(
    ...
    return crud.list_events(db, limit=limit, offset=offset, outcome=outcome)
...
@router.post("", response_model=AuthLogOut, status_code=status.HTTP_201_CREATED)
def create_event(
    payload: AuthLogCreate,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user),
):
